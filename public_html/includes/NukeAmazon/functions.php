 ...
 * BreakNotInLoopOrSwitchToReturnRector (https://stackoverflow.com/questions/3618030/php-fatal-error-cannot-break-continue https://stackoverflow.com/questions/11988281/why-does-cannot-break-continue-1-level-comes-in-php)
...
function amazon_search($SearchType, $SearchTerm, $Mode, $Type, $AMZpage)
{
 ...
 global $db, $amazon_id, $AMZModule_Name, $admin, $prefix, $AMZShowXML, $AMZVer, $amazon_xml_cache_maxtime, $AMZMore, $AMZSingle, $AMZQuickAdd, $AMZShowReview, $AMZShowSimilar, $AMZLocale, $AMZReviewMod, $AMZImgType, $AMZDefault_Asin, $AMZ_Popular, $AMZBuyBox, $AMZCartID;
	...
	$Search = urlencode((string) $SearchTerm);
...
*/
	...
	# No Requested Data is Available. Then use the default ASIN.
		...
		$AMZCount = is_countable($amazon->records) ? count($amazon->records) : 0;
	}
...
# Product Availability
		if ($arr['availability'])
		{
			...
			$Availability = "<b>" . _AMZAVAILABLILITY . ":</b>  " . $AMZAvail ."<br>";
		}
# ISBN
		if ($arr['isbn'])
		{
			$Isbn = "<b>ISBN:</b> " . $arr['isbn'] . "<br>";
		}
# Release Date
		if($arr['releasedate']) {$ReleaseDate="<b>"._AMZRELEASE.":</b> ".AMZFormatDate($arr['releasedate'])."<br>";}
# Age Group
		if($arr['agegroup'])
		{
			$AgeGroup = "<b>"._AMZAGE.":</b> " . $arr['agegroup'];
...
# Catalog Image
		$CatImageUrl = catalog_image(strtolower((string) $ProductCatalog));
...
# Manufacturer search options for valid searches.
		if($arr['manufacturer'])
		{
			...
			$Manufacturer = "<b>".$Manf.":</b> ";
			$PCat = strtolower((string) $ProductCatalog);
			if ($PCat == "photo" || $PCat == "video games" || $PCat == "personal computer" || $PCat == "electronics" || $PCat == "kitchen" || $PCat == "software" )
			{
				...
				$Manufacturer .= AMZ_URL_Verify($arr['manufacturer']) . "&amp;mode=" . Get_Cat_Mode(strtolower((string) $ProductCatalog)) . "\">" . $arr['manufacturer'] . "</a>";
			}
			else
			{
...
# Book Author
		if(is_array($arr['author']))
		{
			...
			while ($AuthorCount > $i)
			{
				...
				$Author .= 	AMZ_URL_Verify($arr['author'][$i]) . "&amp;mode=" . Get_Cat_Mode(strtolower((string) $ProductCatalog)) . "\">" . $arr['author'][$i] . "</a>";
				$i++;
			}
		}
# Music Artist
		if(is_array($arr['artist']))
		{
			...
			while ($ArtistCount > $i)
			{
				...
				$Artist .= AMZ_URL_Verify($arr['artist'][$i]) . "&amp;mode=" . Get_Cat_Mode(strtolower((string) $ProductCatalog)) . "\">" . $arr['artist'][$i] . "</a>";
				$i++;
			}
		}
# Price
		$OurPrice            = $arr['ourprice'];
		if (strtolower((string) $OurPrice) == "too low to display")
		{
			$OurPrice = "" . _AMZTOOLOW . "";
		}
		elseif ($OurPrice == "")
...
# FOR MOVIES
		...
		{
			$Mpaa = explode(" ",(string) $arr['mpaarating']);
			$Rated = "<b>" . _AMZRATED . ":</b> ";
...
# Actors
		if(is_array($arr['actor']))
		{
			...
			while ($ActorCount > $i)
			{
				...
				$Actor .= AMZ_URL_Verify($arr['actor'][$i]) . "&amp;mode=" . Get_Cat_Mode(strtolower((string) $ProductCatalog)) . "\">" . $arr['actor'][$i] . "</a>";
				$i++;
			}
		}
# Directors
		if(is_array($arr['director']))
		{
			...
			while ($DirectorCount > $i)
			{
				...
				$Director .= AMZ_URL_Verify($arr['director'][$i]) . "&amp;mode=" . Get_Cat_Mode(strtolower((string) $ProductCatalog)) . "\">" . $arr['director'][$i] . "</a>";
				$i++;
			}
		}
...
# FOR SOFTWARE / VIDEO GAMES PLATFORM
		if(isset($arr['platform']))
		{
			...
			$Platform   = "<b>" . _AMZPLATFORM . ":</b> " . $PlatformImage . " " . $Platforml;
		}
...
# Sales rank
		...
		{
			$SalesRank = "<b>" . _AMZRANK . ":</b> " . $arr['salesrank'] . "<br>";
		}
# Customer Rating
		...
		if($Rating > 0)
		{
			...
			$AverageRating = "<b>" . _AMZPOP . ":</b> $starimageurl (" . $arr['totalcustomerreviews'] . " " . _AMZTOTALREVIEWS . ")<br>";
		}
# Product URL Link
		if ($MainProd)
		{
...
# Product Browse Bar
		...
		{
			# Home
			$bbtxt = "&nbsp;<b><a href=\"modules.php?name=$AMZModule_Name&amp;op=home\">Home</a> > ";
			# Catalog
			$bbarSQL = "SELECT node FROM ".$prefix."_amazon_nodes WHERE (catalog = '" . 			strtolower((string) $ProductCatalog) . "' and pnode is null and locale = '" . $AMZLocale . "')";
			$result = $db->sql_query($bbarSQL);
			[$catalognode] = $db->sql_fetchrow($result);
			if ($catalognode)
			{
				$cartnode = "BrowseNodeSearch&amp;keyword=$catalognode&amp;mode=".strtolower((string) Get_Cat_Mode($ProductCatalog));
				$bbtxt .= "<a href=\"modules.php?name=$AMZModule_Name&amp;op=" . $cartnode . "\"> " . Get_Cat_Desc($ProductCatalog) . "</a> > ";
			}
			# Featured Product
			if(AsinExist($Asin) && is_admin($admin))
			{
				$bbtxt .= " <a href=\"modules.php?name=$AMZModule_Name&amp;op=ShowFI&amp;catalog=" . 			strtolower((string) $ProductCatalog) . "\">" . _AMZFEATURED . "</a> > ";
			}
			# Product Name
			$bbtxt .= "$ProductName</b>";
...
# Quick Add Ends
		...
		if ($Savings[1] > 0 && !$NoImage && $AMZImgType != "NO")
		{
			if ($Savings[1] > 9)
			{
				$ImageUrl = str_replace (".01.", ".01._PE".$Savings[1]."_".$AMZImgType."_SC", (string) $ImageUrl);
			}
			else
			{
				$ImageUrl = str_replace (".01.", ".01._PE0".$Savings[1]."_".$AMZImgType."_SC", (string) $ImageUrl);
			}
		...
		if ($ListPrice)
		{
			$content .= "<b>" . _AMZLSTPRICE . ":</b> <strike>$ListPrice</strike><br>\n";
		}
		if ($OurPrice)
		{
			$content .= "<font color=\"DarkRed\"><b>" . _AMZPRICE . ": <big>$OurPrice</big></b></font><br>";
		}
		if ($Savings[1] > 0)
		{
			$content .= "<b>" . _AMZSAVE . ":</b> " . AMZPriceFormat($Savings[0], $AMZLocale) . " ($Savings[1]% " . _AMZPERCENT . ")<br>";
		}
...
# Write Rewiew for this product
	...
	# Add to shopping cart
		...
		{
			$cartnode = str_replace("&amp;", ":", (string) $cartnode);
			if ($AMZLocale == 'uk')
			{
...
# Product Description
		...
		if(is_array($arr['feature']))
		{
			...
			$content .= "<table><tr><td><b>Product Features:</b></td></tr>";
			$i = 1;
			$content .= "<tr><td><UL>";
		...
		if ($ProductCatalog == "Music")
		{
			...
			if(is_array($arr['track']))
			{
				...
				while ($TrackHalf + 1 > $i)
				{
					...
					$Track .= "<tr valign=top>";
					$Track .= "<td bgcolor=\"$backcolor\" width = \"15\" align=\"right\"><font color=\"$textcolor\"><b>" . $i . ".</b></td><td bgcolor=\"$backcolor\" width = \"250\"><font color=\"$textcolor\">&nbsp;" . $arr['track'][$i-1];
					$Track .= "</td><td bgcolor=\"$backcolor\" >&nbsp;</td>";
					$TrackTemp = $i + $TrackHalf;
					if ($TrackCount + 1 > $TrackTemp )
					{
						$Track .= "<td bgcolor=\"$backcolor\" width = \"15\" align=\"right\"><font color=\"$textcolor\"><b>";
						$Track .= $TrackTemp . ".</b></td><td bgcolor=\"$backcolor\" width = \"250\"><font color=\"$textcolor\">&nbsp;" . $arr['track'][$i + $TrackHalf - 1];
						$Track .= "</td>";
					}
					else
					{
		...
		if(is_array($arr['accessory']))
		{
			...
			$AccessoryAmazonData = amazon_search('AsinSearch', $keyword, strtolower((string) $ProductCatalog), $AMZType, 1);
			...
			foreach($AccessoryAmazonData->records as $ind => $a_arr)
			{
				...
				if (strtolower((string) $A_OurPrice) == "too low to display")
				{
				...
				else
				{
					...
					$sql = "SELECT no_image FROM ".$prefix."_amazon_catalog WHERE r_catalog = '$catalog'";
					$result = $db->sql_query($sql);
					[$A_ImageUrl] = $db->sql_fetchrow($result);
				}
		...
		if ($AMZShowSimilar)
		{
			if (is_array($arr['product']))
			{
				...
				$SimilarAmazonData = amazon_search('AsinSearch', $keyword, strtolower((string) $ProductCatalog), $AMZType, 1);
				foreach($SimilarAmazonData->records as $ind => $s_arr)
				{
					$S_Asin         = $s_arr['asin'];
					$S_ProductName  = $s_arr['productname'];
					$S_OurPrice     = $s_arr['ourprice'];
					if (strtolower((string) $S_OurPrice) == "too low to display")
					{
				...
				# Product Image
					...
					else
					{
						...
						$sql = "SELECT no_image FROM ".$prefix."_amazon_catalog WHERE r_catalog = '$catalog'";
						$result = $db->sql_query($sql);
						[$S_ImageUrl] = $db->sql_fetchrow($result);
					}
...
# BrowseId
		$content .= "<br>\n";
		$content .= AMZBorder(_AMZBROWSE . " " . Get_Cat_Desc(strtolower((string) $ProductCatalog)) . " " . _AMZSEARCHIN);
		...
		foreach ($arr['browsename'] as $ind => $browsename)
		{
			...
			if ($arr['browseid'][$i])
			{
				...
				$link .= $arr['browseid'][$i];
				$link .= "&amp;mode=". urlencode(strtolower((string) Get_Cat_Mode(strtolower((string) $ProductCatalog)))) . "\">";
			}
			if ($link != "")
			{
				$content .= $link;
			...
			else
			{
				...
				$content .= "<a href=\"modules.php?name=$AMZModule_Name&amp;op=ShowResults&amp;keyword=".urlencode((string) $browsenamelink)."&amp;mode=".urlencode((string) Get_Cat_Mode(strtolower((string) $ProductCatalog)))."\">";
			}
...
# Show previous/more buttons
		...
		$content .= show_sub_catalog(strtolower((string) $ProductCatalog));
	return $content;
}
...
# Converts EUR 1.234,56 to 1234.56
	...
	{
		$item = str_replace(".", "", (string) $item); // replace dots (thousand seps) with blancs
		$item = str_replace(",", ".", $item); // replace ',' with '.'
	}
	if(preg_match("#([0-9\.]+)#", (string) $item, $match))
...
function AMZSearchForm()
{
	...
	echo "<table width=\"100%\" cellpadding=\"1\" cellspacing=\"0\" border=\"0\" bgcolor=\"$bgcolor2\">\n"
		...
		."<INPUT TYPE=\"text\" style=\"background:$bgcolor1;\" NAME=\"keyword\" SIZE=\"24\" maxlength=\"50\" VALUE=\"\">&nbsp;&nbsp;<b>"._AMZSEARCHIN." </b>&nbsp;&nbsp;\n"
		...
		$result = $db->sql_query($sql);
		while ([$ASMode, $ASCat] = $db->sql_fetchrow($result))
		{
...
function AMZ_submit($asin)
{
 ...
 global $db, $admin, $prefix, $AMZModule_Name;
	if (is_admin($admin))
	{
		...
		foreach ($amazon->records as $ind => $arr)
		{
			$ProductCatalog = strtolower((string) $arr['catalog']);
		...
		."<div align=\"center\"><b>$asin</b><br>Has been added<br>\n"
		."($ProductCatalog)<br>\n"
	...
	else
	{
		...
		."<title>$AMZModule_Name: Quick Product Add</title>\n"
		."<font size=\"2\" color=\"#363636\" face=\"Verdana, Helvetica\">\n"
		."<b>Please</b> - Play somewere else...<br>\n"
...
function AMZ_remove($asin)
{
 ...
 global $db, $admin, $prefix;
	if (is_admin($admin))
	{
		...
		."<font size=\"2\" color=\"#363636\" face=\"Verdana, Helvetica\">\n"
		."<div align=\"center\"><b>$asin</b><br>Has been deleted<br>\n"
		."<img src=\"" . AMZIS($asin, 'T') . "\"><br><br>\n"
		."[ <a href=\"javascript:void(0)\" onClick=javascript:self.close()>Close</a> ]</div>\n"
		."</font>\n"
		."</body>\n"
...
# Determine total and quantity of items in Shopping Cart
	...
	$AMZCatalog = explode("+", (string) $AMZCatalog);
	$CatCount = count($AMZCatalog);
	$i = 0;
	while ($i < $CatCount - 1)
	{
		echo "<td style=\"border: 1px solid #000000; background-color:$bgcolor2; text-align: center\"><a style=\"text-decoration: none;color:$textcolor2;\" href=\"modules.php?name=$AMZModule_Name&amp;op=ShowFI&amp;catalog=".urlencode($AMZCatalog[$i]) ."\"><b>" . Get_Cat_Desc($AMZCatalog[$i]) . "</b></a></td>\n";
		echo "<td width=\"2\">&nbsp;</td>\n";
		$i++;
	}
	echo "</tr>\n";
    ...
    echo "&nbsp;<a href=\"modules.php?name=$AMZModule_Name\"><font class=\"title\"><b>$text</b></font></a></td>\n";
	if(is_admin($admin))
	{
...
function get_book_details($asin)
{
 ...
 global $db, $prefix;
	...
	foreach($BookData->records as $ind => $b_arr)
	{
		...
		if (strtolower((string) $b_arr['ourprice']) == "too low to display")
		{
	...
	# Product Image
		...
		else
		{
			...
			$sql = "SELECT no_image FROM ".$prefix."_amazon_catalog WHERE r_catalog = '$catalog'";
			$result = $db->sql_query($sql);
			[$details['image'][$i]] = $db->sql_fetchrow($result);
		}
		$i++;
	}
	return $details;
...
# MarketPlace Seller Profile
		...
		{
			if ((is_countable($arr['sellernickname']) ? count($arr['sellernickname']) : 0)==1)
			{
				...
				$Profile .= "<tr>";
				$Profile .= "<td colspan=\"2\"><b>" . _AMZFEEDBACK . ":</b><p></td></tr>";
				$i =0;
				while ($Feedback > $i)
				{
					$Profile .= "<tr>";
					...
					$Profile .= "<b>" . _DATE . ":</b> " . $arr['feedbackdate'][$i] . "&nbsp;&nbsp;";
					$Profile .= "<b>" . _AMZRATER . ":</b> " . $arr['feedbackrater'][$i] . "<hr>";
					$Profile .= "</td>";
...
# MarketPlace Offer
		if ($arr['numberofofferings'])
		{
			...
			$OfferContents .= "<table width=\"200\" border=\"2\" cellspacing=\"2\" cellpadding=\"1\"><tr><td align=\"center\"><b>". _AMZPRICEGLANCE ."</b><br>";
			if ($ListPrice)
			{
				$OfferContents .= "<b>" . _AMZLSTPRICE . ":</b> <strike>$ListPrice</strike><br>";
			}
			if ($arr['thirdpartynewprice'])
			{
				$OfferContents .=  "<b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=ThirdPartyNew\">" . _AMZNEW . "</a></b> " . _AMZFROM . " ". $arr['thirdpartynewprice'] . "<br>";
			}
			if ($arr['usedprice'])
			{
				$OfferContents .=  "<b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Used\">" . _AMZUSED . "</a></b> " . _AMZFROM .  " " . $arr['usedprice'] . "<br>";
			}
			if ($arr['collectibleprice'])
			{
				$OfferContents .= "<b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Collectible\">" . _AMZCOLLECTIBLE . "</a></b> " . _AMZFROM .  " " . $arr['collectibleprice'] . "<br>";
			}
			if ($arr['refurbishedprice'])
			{
				$OfferContents .= "<b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Refurbished\">" . _AMZREFURBISHED . "</a></b> " . _AMZFROM . " " .  $arr['refurbishedprice'] . "<br>";
			}
...
# Need to create a page system for showing the other pages
			...
			$Offerings = is_countable($arr['offeringprice']) ? count($arr['offeringprice']) : 0;

			$market_condition = ["new" => _AMZNEW, "mint" => _AMZMINT, "good" => _AMZGOOD, "acceptable" => _AMZACCEPTABLE, "verygood" => _AMZVERYGOOD, "refurbished" => _AMZREFURBISHED];

			if ($Offerings > 0)
			{
				$i = 0;
				$OfferContents .= _AMZBROWSEBYT . ":   <b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=All\">" . _AMZALL . "</a></b> ";
				if ($arr['thirdpartynewprice'])
				{
					$OfferContents .= " | <b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=ThirdPartyNew\">". _AMZNEW . "</a></b>";
					$TypeTemp = true;
				}
				if ($arr['usedprice'])
				{
					$OfferContents .= " | <b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Used\">" . _AMZUSED . "</a></b>";
					$TypeTemp = true;
				}
				if ($arr['collectibleprice'])
				{
					$OfferContents .=  " | <b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Collectible\">" . _AMZCOLLECTIBLE . "</a></b>";
					$TypeTemp = true;
				}
				if ($arr['refurbishedprice'])
				{
					$OfferContents .=  " | <b><a href=\"modules.php?name=$AMZModule_Name&amp;op=MarketPlaceSearch&amp;keyword=$Asin&amp;mode=Refurbished\">" . _AMZREFURBISHED . "</a></b>";
				}
				$OfferContents .= "</td></tr></table>";
				$OfferContents .= "<br><table>\n<tr><td width=\"15%\"><b>" . _AMZBLKPRICE . "</b></td>\n";
				$OfferContents .= "<td width=\"20%\" align=\"center\"><b>" . _AMZCONDITION . "</b></td>\n";
				$OfferContents .= "<td width=\"50%\"><b>" . _AMZSELLERINFO . "</b></td>\n";
				...
				while ($Offerings > $i)
				{
					...
					$OfferContents .= "<td valign=\"top\"><b>". $arr['offeringprice'][$i] . "</b></td>\n";

					$OfferContents .= "<td valign=\"top\" align=\"center\">". $market_condition[$arr['condition'][$i]] . "</td>\n";

					$OfferContents .= "<td valign=\"top\"><b>" . _AMZSELLER . ":</b>";
					if ($AMZLocale == 'us')
					{
					...
					$OfferContents .= "<br>\n";
					$OfferContents .= "<b>" . _AMZSHIPS . ":</b> " . $arr['sellerstate'][$i] . ", " . $arr['sellercountry'][$i] . "<br>" . $arr['exchangeavailability'][$i] . "<br>";
					$OfferContents .= "<b>" . _AMZCOMMENTS . ":</b> ". $arr['conditiontype'][$i] . "</td>\n";
...
#	Start Count
		...
		while([$asin] = $db->sql_fetchrow($result))
		{
			if ($count > 0)
			{
				$asin_list .= ",";
...
function catalog_image($catalog)
{
	...
	$result = $db->sql_query($sql);
	[$CatImageUrl] = $db->sql_fetchrow($result);
	return $CatImageUrl;
}
...
function show_catalog()
{
 ...
 global $db, $prefix, $AMZLocale, $AMZModule_Name, $bgcolor2;
	...
	while ([$cat, $node] = $db->sql_fetchrow($result))
	{
		if ($count == 3) {
	    	echo "<tr>\n";
	    	$count = 0;
		...
		$link = "<a href=\"modules.php?name=$AMZModule_Name&amp;op=BrowseNodeSearch&amp;keyword=$node&amp;mode=". urlencode(strtolower((string) Get_Cat_Mode($cat))) . "\">";
		$desc = Get_Cat_Desc($cat);
		echo "$link<img src='" . catalog_image(strtolower((string) $cat)) . "' border = \"0\" align=\"middle\" alt=\"$desc\"></a> $link" . $desc . "</a><br>";
		echo "</td>\n";
		$count++;
		if ($count == 3) {
			echo "</tr>\n";
...
function show_sub_catalog($catalog)
{
 ...
 global $db, $prefix, $AMZLocale, $AMZModule_Name, $bgcolor2;
	...
	$result = $db->sql_query($sql);
	[$AMZLocale, $AMZImgType] = $db->sql_fetchrow($result);
	$c_content = "";
	...
	if ($numrows > 0)
	{
		...
		$count = 0;
		while ([$node, $desc] = $db->sql_fetchrow($result))
		{
			if ($count == 3)
			{
				$c_content .= "<tr>\n";
			...
			$link = "<a href=\"modules.php?name=$AMZModule_Name&amp;op=BrowseNodeSearch&amp;keyword=$node&amp;mode=". urlencode(strtolower((string) Get_Cat_Mode($catalog))) . "\">";
		...
		$asin_show = GetAsin(strtolower((string) $catalog));
		...
		$AMZMainProduct = false;
		$keyword = $asin_show;
		$BookData = amazon_search('AsinSearch', $keyword, strtolower((string) $catalog), $AMZType, 1);
...
# Manufacturer search options for valid searches.
			if($c_arr['manufacturer'])
			{
				...
				$Manufacturer = "<b>".$Manf.":</b> ";
				$PCat = strtolower((string) $catalog);
				if ($PCat == "photo" || $PCat == "video games" || $PCat == "personal computer" || $PCat == "electronics" || $PCat == "kitchen" || $PCat == "software" )
				{
					$Manufacturer .= "<a href=\"modules.php?name=$AMZModule_Name&amp;op=ManufacturerSearch&amp;keyword=" . AMZ_URL_Verify($c_arr['manufacturer']) . "&amp;mode=".strtolower((string) $catalog) . "\">" . $c_arr['manufacturer'] . "</a>";
				}
				else
				{
			...
			$c_content .= $LinkStartStr;
			$c_content .= "<b>$ProductName</b>";
			$c_content .= "</a>";
			$c_content .= "<br>";
			if ($ListPrice)
			{
				$c_content .= "<b>" . _AMZLSTPRICE . ":</b> <strike>$ListPrice</strike><br>";
			}
			if ($OurPrice)
			{
				$c_content .= "<font color=\"DarkRed\"><b>" . _AMZPRICE . ": $OurPrice</b></font><br>";
			}
			if ($Savings[1] > 0)
			{
				$c_content .= "<b>" . _AMZSAVE . ":</b> " . AMZPriceFormat($Savings[0], $AMZLocale);
...
function AMZSimilar($amazon)
{
	...
	$content .= "<b>" . _AMZRELATED . "</b></td></tr>\n";
	$content .= "<tr><td align =\"center\" style=\"background-color:$bgcolor1\">\n";

	$AMZ_Similar_Count = is_countable($amazon->records) ? count($amazon->records) : 0;
	...
	# Extract fields of interest but first clear from previews pass.
		...
		if (strlen((string) $prodName) > 50)
		{
			$prodName = rtrim(substr((string) $prodName, 0, 50)).'&nbsp;...';
		}
...
# Price
		if ($listprice > $ourprice)
		{
			$content .= "" . _AMZBLKLSTPRICE . ": <strike><b>$listprice</b></strike><br>";
			$content .= "" . _AMZBLKPRICE . ": <font color=\"DarkRed\"><b>$OurPrice</b></font><br>";
		}
		else
		{
			 $content .= "" . _AMZBLKPRICE . ": <font color=\"DarkRed\"><b>$OurPrice</b></font><br>";
...
function AMZBorder($text)
{
	...
	$Aborder .= "<font color=\"$textcolor2\"><b>$text</b></font>";
	$Aborder .= "</td></tr></table>";
	$Aborder .= "</td></tr></table>\n";
	return 	$Aborder;
}
...
function node_desc($node)
{
	...
	$result = $db->sql_query($sql);
	[$catalog, $node_desc] = $db->sql_fetchrow($result);
...
function calculate_price($AMZCartID)
{
	...
	else
	{
		...
		$result = $db->sql_query($SQL);
		while([$asin, $quantity] = $db->sql_fetchrow($result))
		{
			$amazon = amazon_search('AsinSearch', $asin, '', 'lite', '');
			foreach ($amazon->records as $ind => $arr)
			{
...
# if we have just added an item to the cart, continue shopping in that category
	...
	$path = $PHP_SELF;
	$path = str_replace("show_cart.php", "", (string) $path);
...
###########################################
	...
	if (!$AMZ_No_CheckOut)
	{
		if ($AMZLocale == 'us')
		{
			...
			$start = "<PurchaseUrl>";
			$end = "</PurchaseUrl>";
			$start_position = strpos((string) $String, $start) + strlen($start);
			$end_position = strpos((string) $String, $end);
			$length = $end_position - $start_position;

			$PurchaseUrl = trim(substr((string) $String, $start_position, $length));
		...
		else
		{
			...
			$SQL = "SELECT asin, quantity FROM ". $prefix . "_amazon_cart WHERE session='$AMZCartID'";
			$result = $db->sql_query($SQL);
			[$asin, $quantity] = $db->sql_fetchrow($result);
			if ($AMZLocale == 'uk')
			{
...
# Check if xml is availible and cache age is not larger than defined
	...
	else
	{
		...
		if ($String != "0")
		{
			$sql = "insert into " . $prefix . "_amazon_cache values (NULL, now(), '$LocalFile', '".addslashes((string) $String)."' )";
			$result = $db->sql_query($sql);
		}
		return $String;
	}
...
function AMZ_file_get_contents($filename)
{
 ...
 $fp = @fopen($filename, "r");
	...
	while (!feof($fp))
	{
		$temp .= mb_convert_encoding(fread($fp, 4096), 'ISO-8859-1');
	}
	fclose($fp);
	return $temp;
}
