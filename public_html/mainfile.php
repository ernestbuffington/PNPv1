...
# Create REQUEST_URI for IIS servers
...
function addCSSToHead($content, $type = 'file') {
    ...
    if (($type == 'file') && (is_array($headCSS) && $headCSS !== []) && (in_array([$type, $content], $headCSS))):
        return;
    endif;
...
function addJSToHead($content, $type = 'file') {
    ...
    if (($type == 'file') && (is_array($headJS) && $headJS !== []) && (in_array([$type, $content], $headJS))):
        return;
    endif;
...
function validate_mail($email) {
    //if(strlen($email) < 7 || !preg_match('/\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/', $email)): TheGhost
