...
#	Start Count
		while([$iid, $asin, $hits, $category] = $db->sql_fetchrow($result))
		{
			if ($count == 3) {
		    	echo "<tr>\n";
		    	$count = 0;
...
# This function gets all catalogs and finds the number of featured items
	...
	while([$catalog] = $db->sql_fetchrow($result))
	{
		...
		[$totalcatalog] = $db->sql_fetchrow($resultm);
...
//    //GraphicAdmin();
	...
	if ($total_rows>0)
	{
		...
		."</tr><tr>";
		while ([$iid, $asin, $imp, $hits, $clicks] = $db->sql_fetchrow($result))
		{
			if($imp==0) {
				$hpercent = "0 %";
			} else {
...
//    //GraphicAdmin();
	...
	if ($total_rows>0)
	{
		...
		."<td bgcolor=\"$bgcolor2\" align=\"center\"><strong>Action</strong></td>"
		."</tr><tr>";
		while ([$iid, $asin, $hits, $clicks] = $db->sql_fetchrow($result))
		{
			if($hits==0) {
				$cpercent = "0 %";
			} else {
...
//    //GraphicAdmin();
	...
	$result = $db->sql_query($AMZ_SQL);
	[$AMZ_total_items, $AMZ_Imp, $AMZ_Hits, $AMZ_Clicks, $AMZ_MI, $AMZ_MH, $AMZ_MC] = $db->sql_fetchrow($result);
	$AMZ_SQL = "select count(*), sum(hits), sum(clicks), max(hits), max(clicks) from ".$prefix."_amazon_not_item";
	$result = $db->sql_query($AMZ_SQL);
	[$AMZ_total_items_nf, $AMZ_Hits_nf, $AMZ_Clicks_nf, $AMZ_MH_N, $AMZ_MC_N] = $db->sql_fetchrow($result);
...
switch ($op){
	...
	if(!isset($AMZModule_Name))
	$AMZModule_Name = '';

	if(!isset($amazon_id))
	$amazon_id = '';

	if(!isset($amazon_xml_cache_maxtime))
	$amazon_xml_cache_maxtime = '';
	...
	if(!isset($AMZQuickAdd))
	$AMZQuickAdd = '';

	if(!isset($AMZShowReview))
	$AMZShowReview = '';

	if(!isset($AMZShowSimilar))
	$AMZShowSimilar = '';

	if(!isset($AMZReviewMod))
	$AMZReviewMod = '';

	if(!isset($AMZImgType))
	$AMZImgType = '';

	if(!isset($AMZDefault_Asin))
	$AMZDefault_Asin = '';

	if(!isset($AMZ_Popular))
	$AMZ_Popular = '';
    ...
    break;
	case "amazon_rpt":
    amz_report(isset($page));
    break;
	case "amazon_rpt_nf":
    amz_report_nf(isset($page));
    break;
	case "amazon_reset":
    am_reset_stats();
    break;
