...
include("includes/functions_canned.php");
//
// Is PM disabled?
//
...
$buddy_action = ( isset($HTTP_GET_VARS['buddy_action']) ) ? ( ($HTTP_GET_VARS['buddy_action'] == 'add') ? 'add' : ( ($HTTP_GET_VARS['buddy_action'] == 'remove') ? 'remove' : '') ) : '';
/*****************************************************/
...
/*****************************************************/
	if ( $privmsg['user_session_time_1'] >= (time()-3600) )
	{
		...
		{
			$online_status = '&nbsp;(<b><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_online'], $username_from) . '"' . $online_color . '>' . $lang['Online'] . '</a></b>)';
		}
		else if ( ( $userdata['user_level'] == ADMIN ) || ( $userdata['user_id'] == $user_id_from ) )
		{
			$online_status = '&nbsp;(<b><i><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_hidden'], $username_from) . '"' . $hidden_color . '>' . $lang['Hidden'] . '</a></i></b>)';
		}
		else
		{
			$online_status = '&nbsp;(<b><span title="' . sprintf($lang['is_offline'], $username_from) . '"' . $offline_color . '>' . $lang['Offline'] . '</span></b>)';
		}
	}
	else
	{
	...
	if ( $privmsg['user_session_time_2'] >= (time()-3600) )
	{
		...
		{
			$online_status_2 = '&nbsp;(<b><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_online'], $tusername_to) . '"' . $online_color . '>' . $lang['Online'] . '</a></b>)';
		}
...
/*****************************************************/
    include("includes/ropm_quick_reply.php");
/*****************************************************/
...
/*****************************************************/
	...
	// Canned MOD Begin
	$s_canned_select = get_canned_select(true);
	get_canned_info($canned,$privmsg_message,$privmsg_bbcode_enabled,$move,$lock);
	// Canned MOD End
        $template->assign_vars(array(
...
/* Forum - Buddy List v.0.9.5                  START */
/*****************************************************/
include('includes/mini_buddylist.'.$phpEx);
/*****************************************************/
/* Forum - Buddy List v.0.9.5                    END */
/*****************************************************/
$template->pparse('body');
