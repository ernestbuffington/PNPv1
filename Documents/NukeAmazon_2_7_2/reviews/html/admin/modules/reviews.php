...
function reviews() {
    ...
    if ($numrows>0) {
	while(list($id, $date, $title, $text, $reviewer, $email, $score, $url, $url_title, $rlanguage, $asin) = $db->sql_fetchrow($result)) {
	    $id = intval($id);
	    $score = intval($score);
		...
		echo "<input type=\"hidden\" name=\"asin\" value=\"$asin\">";
...
function add_review($id, $date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $rlanguage, $asin) {
    global $prefix, $db;
    $id = intval($id);
    $title = stripslashes(FixQuotes($title));
    ...
    $score = intval($score);
    $db->sql_query("insert into ".$prefix."_reviews values (NULL, '$date', '$title', '$text', '$reviewer', '$email', '$score', '$cover', '$url', '$url_title', '1', '$rlanguage', '$asin')");
...
switch ($op){
    ...
    case "add_review":
    add_review($id, $date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $rlanguage, $asin);
    break;
